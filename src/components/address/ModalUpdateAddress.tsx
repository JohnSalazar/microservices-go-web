...
type ModalUpdateAddressProps = {
  ...
  address: AddressType
  updateAddressFromAddressesList: (address: AddressType) => Promise<void>
}
...
export default function ModalUpdateAddress({
  ...
  address,
  updateAddressFromAddressesList,
}: ModalUpdateAddressProps) {
  const { customerService } = useApi()
  const { UpdateAddress } = customerService
  ...
  const update = async (values: AddressType) => {
    setLoading(true)
    await UpdateAddress(values)
      .then((res) => {
        updateAddressFromAddressesList(res)
        setLoading(false)
        setOpenModalUpdateAddress(false)
      })
      .catch((err) => {
